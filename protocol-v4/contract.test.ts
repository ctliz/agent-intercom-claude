...
test("Claude consumes the reviewed Core protocol-v4 corpus", () => {
...
test("scope parser is exact and unscoped is strict", () => {
  ...
  assert.throws(() => parseIntercomScopeId("Scope.AAAAAAAAAA"), /must match/);
  assert.throws(() => parseIntercomScopeId("éAAAAAAAAAAAAAAA"), /must match/);
  assert.throws(() => parseIntercomScopeId(42 as any), /must match/);
...
test("scope lower and upper bound accepted", () => {
  assert.equal(parseIntercomScopeId("A".repeat(16)), "A".repeat(16));
  assert.equal(parseIntercomScopeId("A".repeat(128)), "A".repeat(128));
});

test("scope case is preserved exactly", () => {
  const mixed = "Scope_AaBbCcDdEe";
  assert.equal(parseIntercomScopeId(mixed), mixed);
});
